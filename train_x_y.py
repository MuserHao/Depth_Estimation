...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        outputs = beta(inputs)
        targets = beta_true(inputs)


        # net.conv1.weight.data = targets.view(100, 3, 3, 3)

        Y_samples = langevin_sampling(outputs, net, steps=step_mc, burn_in=step_mc - 10, batch=batch_size) #,
                                    #  initial=torch.rand([1, 3, 32, 32], dtype=torch.float32, device=device))
        Y_true = langevin_sampling(targets, net, step_size=0.00001, steps=20000, burn_in=20000 - 10, batch=batch_size)
        ...
        loss = -(torch.dot(net(Y_true).view(-1), outputs.view(-1)) - torch.dot(net(Y_samples).view(-1), outputs.view(-1)))
        weight_loss_true = criterion(outputs, targets)
        loss.backward()
        optimizer2.step()
        # weight_loss = criterion(outputs, net.conv1.weight.data.view(-1))
        # weight_loss.backward()
        # optimizer2.step()
        image_loss = criterion(Y_samples, Y_true)

        train_image_loss += image_loss.item()
        train_loss += loss.item()
        train_weight_loss += weight_loss_true.item()

        print('Loss: %.6f | Weight loss: %.3f | Image loss: %.6f'
              % (train_loss / (batch_idx + 1), train_weight_loss / (batch_idx + 1), image_loss.item()))
